...
Public Class frmProcessMercuryDataSept
...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.Name = "frmProcessMercuryDataSept"
            ...
            For i = 0 To (lines - 1)
                ...
                If Mid(LineStr, 1, 1) = "1" Then
                    ...
                    With myComm
                        ...
                        z = z & "where refdate = dateadd(mm, 1, '" & Trim(refDat) & "') "
                        .CommandText = z
                ...
                If Mid(LineStr, 1, 1) = "2" Then
                    BrStr = Mid(LineStr, 2, 4)
                    ItmStr = Mid(LineStr, 6, 6)
                    QtyStr = Mid(LineStr, 12, 5)
                End If
                If Trim(mDate) <> "" And Trim(BrStr) <> "" And Trim(ItmStr) <> "" And Trim(QtyStr) <> "" Then
                    With myComm
                        ...
                        z = z & "(branchcode, mitemcode, qty, refdate, arefdate) values "
                        z = z & "('" & Trim(BrStr) & "', '" & Trim(ItmStr) & "', " & Trim(QtyStr) & ", "
                        z = z & "dateadd(mm, 1, '" & Trim(mDate) & "'), '" & Trim(mDate) & "' )"
                        .CommandText = z
                        .CommandType = CommandType.Text
                        .CommandTimeout = 3000
                        .ExecuteScalar()    'NonQuery()
                        myConn.Close()
                        lWrit = lWrit + 1
                    End With
                End If
...
nxt:        Next i
    ...
    Private Sub btnProcess_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnProcess.Click
        ...
        s = s & "where refdate = dateadd(mm, 1, '" & Trim(refDat) & "') "
        ...
        If Trim(SqlValue.Text) <> "" Then
            If MessageBox.Show(DateAdd(DateInterval.Month, 1, CDate(refDat)).ToString("MMMM yyyy") & _
                              " Records exsists! APPEND THIS EXTRACTION TO THE TABLE? ", _
                               DateAdd(DateInterval.Month, 1, CDate(refDat)).ToString("MM/dd/yyyy") & " records exsists!", MessageBoxButtons.YesNo, _
        ...
        With myComm
            myConn.ConnectionString = ServerPath2
            myConn.Open()
            .Connection = myConn
            ProgBar.Value = 1
            ...
            With myParameter
                ...
                .Size = 10
                .Value = DateAdd(DateInterval.Month, 1, CDate(refDat))
            End With
            .CommandText = "MercTemp2Live_Ins"
        ...
        'Dim myRefDat As New SqlParameter("@RefDate", SqlDbType.VarChar, 25)
        'myRefDat.Direction = ParameterDirection.Input
        'myRefDat.Value = Trim(refDat)
        'myParam = myRefDat
        ...
        DataMaintenance.myParamArray(0, 5) = DateAdd(DateInterval.Month, 1, CDate(refDat))
        ProgBar.Value = 5
        'Check Segment Rule here
        If ConfigurationSettings.AppSettings("UseSegmentRule") = "0" Then
            DataMaintenance.ExecuteCommand(ServerPath2, "UpdMercurySale", CommandType.StoredProcedure, 1)
        Else
            DataMaintenance.ExecuteCommand(ServerPath2, "SegmentRule_UpdMercurySale", CommandType.StoredProcedure, 1)
        End If
        ...
        'z = z & " inner join supmr         ff on ee.mrcode      = ff.mrcode and "
        'z = z & "                                aa.refdate between ff.start_date and ff.end_date "
        'z = z & "where aa.refdate = '" & Trim(refDat) & "' "
        'z = z & "  and left(cc.itemcode,3) not in ('164', '171', '172') "
        '.CommandText = "UpdMercurySale"
        '.CommandType = CommandType.StoredProcedure
        '.CommandTimeout = 3000
        'ProgBar.Value = 5
        ...
        SPRefdat.Value = DateAdd(DateInterval.Month, 1, CDate(refDat))
        Prams(0) = SPRefdat
        ...
        z = z & "        and mitemcode in ( select mitemcode from newscores..mercurysaledraft where refdate = '" & DateAdd(DateInterval.Month, 1, CDate(refDat)) & "' ) ) a "
        ...
        z = z & "where ( left(ltrim(rtrim(c.itemcode)),3) in ('164', '171', '172', '122', '117')"
        ...
        RE_eq.Text = FindValue(z)      ''SqlValue.Text
        ...
        RecordSubDistSales(DateAdd(DateInterval.Month, 1, CDate(refDat)))
        'End If
    End Sub
