...
Public Class frmProcessCallRateProduct
...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.Name = "frmProcessCallRateProduct"
        Me.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen
        Me.Text = "Process Call Rate Product"
...
#End Region
    ...
    Private Sub Extract_ExcelFileData(ByVal strFileName As String)
        ...
        Dim PMRCode, PMRName, TeamDivision, ProductLine, DSMCode, DSMName, NSMCode, NSMName, DivCode, DivisionName As String
        ...
        Try
            ...
            With ws
                Do
                    ...
                    TeamDivision = CStr(.Range("D" & i).Text) : If TeamDivision = "" Then Throw New System.Exception("TeamDivision not Found at rownumber : " & i.ToString)
                    ProductLine = CStr(.Range("E" & i).Text) : If ProductLine = "" Then Throw New System.Exception("ProductLine not Found at rownumber : " & i.ToString)
                    CallRate = CStr(.Range("F" & i).Text) : If CallRate = "" Then Throw New System.Exception("CallRate Amount not Found at rownumber : " & i.ToString)
                    Target = CStr(.Range("G" & i).Text) : If Target = "" Then Throw New System.Exception("Target Amount not Found at rownumber : " & i.ToString)
                    DSMCode = CStr(.Range("H" & i).Text) : If DSMCode = "" Then Throw New System.Exception("DSM Code not Found at rownumber : " & i.ToString)
                    DSMName = CStr(.Range("I" & i).Text)
                    NSMCode = CStr(.Range("J" & i).Text) : If NSMCode = "" Then Throw New System.Exception("NSM Code not Found at rownumber : " & i.ToString)
                    NSMName = CStr(.Range("K" & i).Text)
                    DivCode = CStr(.Range("L" & i).Text) : If DivCode = "" Then Throw New System.Exception("Div Code not Found at rownumber : " & i.ToString)
                    DivisionName = CStr(.Range("M" & i).Text)
                    CommissionDate = CStr(.Range("N" & i).Text) : If CommissionDate = "" Then Throw New System.Exception("CommissionDate not Found at rownumber : " & i.ToString)

                    Insert_ExcelFile(PMRCode, PMRName, TeamDivision, ProductLine, CallRate, Target, DSMCode, DSMName, NSMCode, NSMName, DivCode, DivisionName, CommissionDate)
                    i = i + 1
                Loop Until PMRCode = "" And PMRName = "" And TeamDivision = "" And ProductLine = "" And CallRate = "" And Target = "" And DSMCode = "" And DSMName = "" And NSMCode = "" And NSMName = ""
    ...
    Private Sub Insert_ExcelFile(ByVal ParamArray SubData() As String)
        ...
        cm.CommandText = "Insert_CallRateProduct"
        ...
        Dim teamdivision As New SqlParameter("@teamdivision", SqlDbType.VarChar, 50) : teamdivision.Direction = ParameterDirection.Input : cm.Parameters.Add(teamdivision)
        Dim productline As New SqlParameter("@productline", SqlDbType.VarChar, 20) : productline.Direction = ParameterDirection.Input : cm.Parameters.Add(productline)
        ...
        teamdivision.Value = SubData(2).ToString
        productline.Value = SubData(3).ToString
        callrate.Value = CDbl(SubData(4))
        target.Value = CDbl(SubData(5))
        dsmcode.Value = SubData(6).ToString
        dsmname.Value = SubData(7).ToString
        nsmcode.Value = SubData(8).ToString
        nsmname.Value = SubData(9).ToString
        divcode.Value = SubData(10).ToString
        divisionname.Value = SubData(11).ToString
        commissiondate.Value = SubData(12).ToString
    ...
    Private Sub Initialize_CallRateData()
        ...
        With myCommand
            ...
            .CommandText = "CallRate_InitializeProduct"
            .CommandType = CommandType.StoredProcedure
            .Parameters.Add(myParameter)
            Try
                .ExecuteNonQuery()
            Catch ex As Exception
                MessageBox.Show(ex.Message, "Initilize CallRate Product")
            Finally
                myConnection.Close()
            End Try
        End With
