...
Public Class frmProcessSubDistributorData
...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.Name = "frmProcessSubDistributorData"
        Me.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen
        Me.Text = "Process Sub-Distributor Data"
...
#End Region
    ...
    Private Sub btnProcess_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnProcess.Click
        ...
        If txtFileName.Text = "" Then 'added rnj_06262012**return nothing if value is null
            Return
        Else
    ...
    Private Function UpdateFromFileField() As Boolean
        ...
        If CommissionDate > "" Then
            ...
            UpdateString = "Delete from SubDistributorData where FromFile=" & "'" & strFromFile & "'"
            DataMaintenance.ExecuteCommand(ServerPath2, UpdateString, CommandType.Text)

            'Upload Parameters
            UpdateString = "FixSubDistributorRawData"
        ...
        If chkProcess.CheckState = CheckState.Checked Then
            ...
            If ConfigurationSettings.AppSettings("UseSegmentRule") = "0" Then
                UpdateString = "ProcessData_SubDistributorData"
            Else
                UpdateString = "SegmentRule_ProcessData_SubDistributorData"
            End If
    ...
    Private Sub GetNewCustomers(ByVal ResultType As String)
        Select Case ResultType
            Case "Count"
                UpdateString = "Select count(FromFile) from SubDistributorData where " & _
            ...
            Case "Rows"
                ...
                "Address1,Address2 from SubDistributorData where CustCode not in" & _
    ...
    Private Sub GetNewItems(ByVal ResultType As String)
        Select Case ResultType
            Case "Count"
                ...
                "FromFile = " & " '" & strFromFile & "'" & " and DistCode>''"
            ...
            Case "Rows"
                ...
                "listprice [UnitPrice] from SubDistributorData where itemno not in" & _
    ...
    Private Sub GetRowCount(ByVal CountType As String)
        UpdateString = "select SalesClass,Case sd.DistCode " & _
                       "when '01' then 'Barr-Xsel Trading' " & _
                       "when '02' then 'GreenStone Trading' " & _
                       "when '03' then 'Zcab Distributors' " & _
                       "when '04' then 'Globo Asiatico' " & _
                       "when '05' then 'GB Distributors, Inc.' " & _
                       "when '06' then 'Probiotic' " & _
                       "when '07' then 'South Star Drug-MSD' " & _
                       "when '08' then 'Manson Drug' " & _
                       "when '09' then 'Chiral Subdistributor' " & _
                       "when '10' then 'Chiral Sales Allocation' " & _
                       "when '11' then 'Tridharma' " & _
                       "when '12' then 'South Star Drug Transfer' " & _
                       "when '13' then 'Watsons Transfer' " & _
                       "when '14' then 'H.B. Calleja' " & _
                       "when '17' then 'Rose PharmacyTransfer' " & _
                       "when '18' then 'Care Line Enterprises' " & _
                       "when '19' then 'Intermed Pharma Medical' " & _
                       "when '20' then 'Janissa Business Ventures, Inc.' " & _
                       "when '21' then 'Medlink Marketing' " & _
                       "when '22' then 'ACTIMED' " & _
                       "when '23' then 'St. Joseph' " & _
                       "end [SubSaleType],sc.[Name] [SubSubSaleType] " & _
                       "from SubDistributorData sd inner join SubDistributorCodes sc " & _
                       "on sd.distcode=sc.[Dist Code] and sd.SubDistCode=sc.SubDistCode " & _
                       " where FromFile = " & " '" & strFromFile & "'" & _
                       " group by SalesClass, sd.DistCode, sc.[name] "
        Dim myResultSet As SqlDataReader
        myResultSet = DataMaintenance.GetResultSet(ServerPath2, UpdateString, CommandType.Text)
        If myResultSet.HasRows = True Then
            While myResultSet.Read
                SalesClass = CStr(myResultSet("SalesClass"))
                SubSaleType = CStr(myResultSet("SubSaleType"))
                SubSubSaleType = CStr(myResultSet("SubSubSaleType"))
            End While
        End If
        ...
        Select Case CountType
            ...
            Case "Processed"
                UpdateString = "Select count(Rowid) from IntegratedSales where " & _
                               ...
                               " and SaleType='SubDistributor' " & _
                               " and SubSaleType=" & "'" & SubSaleType & "'" & _
                               " and SubSubSaleType=" & "'" & SubSubSaleType & "'"
    ...
    Private Sub GetPesoAmounts(ByVal AmountType As String)
        Select Case AmountType
            Case "Input"
                UpdateString = "Select sum(ValAmt) from SubDistributorData where " & _
                               "FromFile = " & " '" & strFromFile & "'"
    ...
    Private Sub Extract_ExcelFileData(ByVal strFileName As String)
        ...
        Dim distcode, subdistcode, custcode, custname, address1, address2, shiptoname, shipaddress1, shipaddress2 As String
        ...
        Try
            ...
            With ws
                Do
                    distcode = CStr(.Range("A" & i).Text)
                    subdistcode = CStr(.Range("B" & i).Text)
                    custcode = CStr(.Range("C" & i).Text) : If custcode = "" And distcode > "" Then Throw New System.Exception("Customer Code not Found at rownumber : " & i.ToString)
                    If custcode = "" Then Exit Do
                    'Trim Customer Code
                    custcode = custcode.Trim
                    custname = CStr(.Range("D" & i).Text)
                    address1 = CStr(.Range("E" & i).Text)
                    address2 = CStr(.Range("F" & i).Text)
                    shiptoname = CStr(.Range("G" & i).Text)
                    shipaddress1 = CStr(.Range("H" & i).Text)
                    shipaddress2 = CStr(.Range("I" & i).Text)
                    refcode = CStr(.Range("J" & i).Text)
                    refdate = CStr(.Range("K" & i).Text) : If refdate = "" Then Throw New System.Exception("RefDate not Found at rownumber : " & i.ToString)
                    socode = CStr(.Range("L" & i).Text)
                    xrefcode = CStr(.Range("M" & i).Text)
                    remarks = CStr(.Range("N" & i).Text)
                    princode = CStr(.Range("O" & i).Text)
                    itemno = CStr(.Range("P" & i).Text)
                    'Trim Item Code
                    itemno = itemno.Trim
                    itemdesc = CStr(.Range("Q" & i).Text)
                    um = CStr(.Range("R" & i).Text)
                    lotno = CStr(.Range("S" & i).Text)
                    expdate = CStr(.Range("T" & i).Text) : If expdate = "" Then Throw New System.Exception("ExpDate not Found at rownumber : " & i.ToString)
                    qtyorder = CStr(.Range("U" & i).Text) : If qtyorder = "" Then Throw New System.Exception("QtyOrder not Found at rownumber : " & i.ToString)
                    qtyfree = CStr(.Range("V" & i).Text) : If qtyfree = "" Then Throw New System.Exception("QtyFree not Found at rownumber : " & i.ToString)
                    listprice = CStr(.Range("W" & i).Text) : If listprice = "" Then Throw New System.Exception("ListPrice not Found at rownumber : " & i.ToString)
                    valamt = CStr(.Range("X" & i).Text) : If valamt = "" Then Throw New System.Exception("ValAmt not Found at rownumber : " & i.ToString)
                    Insert_ExcelFile(distcode, subdistcode, custcode, custname, address1, address2, shiptoname, shipaddress1, _
                    shipaddress2, refcode, refdate, socode, xrefcode, remarks, princode, itemno, itemdesc, um, lotno, expdate, _
                    qtyorder, qtyfree, listprice, valamt)
                    i = i + 1
                Loop Until custcode = ""
            End With
    ...
    Private Sub Insert_ExcelFile(ByVal ParamArray SubData() As String)
        ...
        cm.CommandText = "Insert_SubDistributorData"
        cm.CommandType = CommandType.StoredProcedure
        Dim distcode As New SqlParameter("@DistCode", SqlDbType.VarChar, 10) : distcode.Direction = ParameterDirection.Input : cm.Parameters.Add(distcode)
        Dim subdistcode As New SqlParameter("@SubDistCode", SqlDbType.VarChar, 25) : subdistcode.Direction = ParameterDirection.Input : cm.Parameters.Add(subdistcode)
        Dim custcode As New SqlParameter("@CustCode", SqlDbType.VarChar, 25) : custcode.Direction = ParameterDirection.Input : cm.Parameters.Add(custcode)
        Dim custname As New SqlParameter("@CustName", SqlDbType.VarChar, 150) : custname.Direction = ParameterDirection.Input : cm.Parameters.Add(custname)
        Dim address1 As New SqlParameter("@Address1", SqlDbType.VarChar, 100) : address1.Direction = ParameterDirection.Input : cm.Parameters.Add(address1)
        Dim address2 As New SqlParameter("@Address2", SqlDbType.VarChar, 100) : address2.Direction = ParameterDirection.Input : cm.Parameters.Add(address2)
        Dim shiptoname As New SqlParameter("@ShipToName", SqlDbType.VarChar, 150) : shiptoname.Direction = ParameterDirection.Input : cm.Parameters.Add(shiptoname)
        Dim shipaddress1 As New SqlParameter("@ShipAddress1", SqlDbType.VarChar, 100) : shipaddress1.Direction = ParameterDirection.Input : cm.Parameters.Add(shipaddress1)
        Dim shipaddress2 As New SqlParameter("@ShipAddress2", SqlDbType.VarChar, 100) : shipaddress2.Direction = ParameterDirection.Input : cm.Parameters.Add(shipaddress2)
        Dim refcode As New SqlParameter("@RefCode", SqlDbType.VarChar, 25) : refcode.Direction = ParameterDirection.Input : cm.Parameters.Add(refcode)
        Dim refdate As New SqlParameter("@RefDate", SqlDbType.VarChar, 25) : refdate.Direction = ParameterDirection.Input : cm.Parameters.Add(refdate)
        Dim socode As New SqlParameter("@SoCode", SqlDbType.VarChar, 25) : socode.Direction = ParameterDirection.Input : cm.Parameters.Add(socode)
        Dim xrefcode As New SqlParameter("@XrefCode", SqlDbType.VarChar, 25) : xrefcode.Direction = ParameterDirection.Input : cm.Parameters.Add(xrefcode)
        Dim remarks As New SqlParameter("@Remarks", SqlDbType.VarChar, 150) : remarks.Direction = ParameterDirection.Input : cm.Parameters.Add(remarks)
        Dim princode As New SqlParameter("@PrinCode", SqlDbType.VarChar, 25) : princode.Direction = ParameterDirection.Input : cm.Parameters.Add(princode)
        Dim itemno As New SqlParameter("@ItemNo", SqlDbType.VarChar, 25) : itemno.Direction = ParameterDirection.Input : cm.Parameters.Add(itemno)
        Dim itemdesc As New SqlParameter("@ItemDesc", SqlDbType.VarChar, 100) : itemdesc.Direction = ParameterDirection.Input : cm.Parameters.Add(itemdesc)
        Dim um As New SqlParameter("@Um", SqlDbType.VarChar, 25) : um.Direction = ParameterDirection.Input : cm.Parameters.Add(um)
        Dim lotno As New SqlParameter("@LotNo", SqlDbType.VarChar, 25) : lotno.Direction = ParameterDirection.Input : cm.Parameters.Add(lotno)
        Dim expdate As New SqlParameter("@ExpDate", SqlDbType.DateTime, 10) : expdate.Direction = ParameterDirection.Input : cm.Parameters.Add(expdate)
        Dim qtyorder As New SqlParameter("@QtyOrder", SqlDbType.Money) : qtyorder.Direction = ParameterDirection.Input : cm.Parameters.Add(qtyorder)
        Dim qtyfree As New SqlParameter("@QtyFree", SqlDbType.Money) : qtyfree.Direction = ParameterDirection.Input : cm.Parameters.Add(qtyfree)
        Dim listprice As New SqlParameter("@ListPrice", SqlDbType.Money) : listprice.Direction = ParameterDirection.Input : cm.Parameters.Add(listprice)
        Dim valamt As New SqlParameter("@ValAmt", SqlDbType.Money) : valamt.Direction = ParameterDirection.Input : cm.Parameters.Add(valamt)

        distcode.Value = SubData(0).ToString
        subdistcode.Value = SubData(1).ToString
        custcode.Value = SubData(2).ToString
        custname.Value = SubData(3).ToString
        address1.Value = SubData(4).ToString
        address2.Value = SubData(5).ToString
        shiptoname.Value = SubData(6).ToString
        shipaddress1.Value = SubData(7).ToString
        shipaddress2.Value = SubData(8).ToString
        refcode.Value = SubData(9).ToString
        refdate.Value = CDate(SubData(10))
        socode.Value = SubData(11).ToString
        xrefcode.Value = SubData(12).ToString
        remarks.Value = SubData(13).ToString
        princode.Value = SubData(14).ToString
        itemno.Value = SubData(15).ToString
        itemdesc.Value = SubData(16).ToString
        um.Value = SubData(17).ToString
        lotno.Value = SubData(18).ToString
        expdate.Value = CDate(SubData(19))
        qtyorder.Value = CDbl(SubData(20))
        qtyfree.Value = CDbl(SubData(21))
        listprice.Value = CDbl(SubData(22))
        valamt.Value = CDbl(SubData(23).ToString)
        Try
            cn.Open()
            cm.Connection = cn
            cm.ExecuteNonQuery()
