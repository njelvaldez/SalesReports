...
Public Class frmGloboSubDist
    ...
    Private Sub InitialPath()
        Dim salesinvupdpath As String = System.Configuration.ConfigurationSettings.AppSettings.Item("GloboRAWPath")
        Dim currentdate As String
    ...
    Private Sub SetPath()
        ...
        txtOutput.Text = txtOutput.Text.ToString().Replace(".xls", "") + "GLOBO RAW DATA " + currentdate + ".xls"
    End Sub
    ...
    Private Sub DisplayColorMapping()
        ...
        For Each row As DataGridViewRow In dgvSales.Rows
            If Not row.IsNewRow Then
                ...
                itemcodestr = row.Cells(6).Value.ToString
                ...
                customercodestr = row.Cells(1).Value.ToString
    ...
    Private Function GetItemCode(ByVal itemcode As String) As String
        ...
        Dim SearchName As New SqlParameter("@SearchName", SqlDbType.VarChar, 50) : SearchName.Direction = ParameterDirection.Input : SearchName.Value = "GLOBO ITEM" : Params(0) = SearchName
    ...
    Private Function GetCustomerCode(ByVal itemcode As String) As String
        Dim BusinessObject As New BusinessLayer.clsFileMaintenance
        Dim Params(1) As SqlParameter
        Dim SearchName As New SqlParameter("@SearchName", SqlDbType.VarChar, 50) : SearchName.Direction = ParameterDirection.Input : SearchName.Value = "GLOBO CUSTOMER" : Params(0) = SearchName
        Dim SearchCode As New SqlParameter("@SearchCode", SqlDbType.VarChar, 50) : SearchCode.Direction = ParameterDirection.Input : SearchCode.Value = itemcode : Params(1) = SearchCode
        ResetTable()
        BusinessObject.Sub_Show(ServerPath2, "Util_DataMap_Select", CommandType.StoredProcedure, RemoteDataSet, "Table1", Params)
        If RemoteDataSet.Tables("Table1").Rows.Count > 0 Then
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        If row > -1 Then
            myLoadedForm.CustomerCode = dgvSales.Rows(row).Cells(1).Value.ToString
            myLoadedForm.CustomerName = dgvSales.Rows(row).Cells(2).Value.ToString
            myLoadedForm.ItemCode = dgvSales.Rows(row).Cells(6).Value.ToString
            myLoadedForm.ItemName = dgvSales.Rows(row).Cells(7).Value.ToString
            
            'Set DataMap Search column
            myLoadedForm.gbCustomer.Text = "GLOBO CUSTOMER"
            myLoadedForm.gbItem.Text = "GLOBO ITEM"
            myLoadedForm.txtUnit.Text = dgvSales.Rows(row).Cells(10).Value.ToString
            myLoadedForm.txtUnitPrice.Text = dgvSales.Rows(row).Cells(13).Value.ToString
            ...
            If Not row.IsNewRow Then
                ...
                If itemcodestr <> "" Then
                    ...
                    With ws
                        ...
                        .Range("A" & rowcounter.ToString()).Value = "04"
                        .Range("B" & rowcounter.ToString()).Value = "GLOBO"
                        .Range("O" & rowcounter.ToString()).Value = "MIC"
                        'A:Invoice No 
                        .Range("J" & rowcounter.ToString()).Value = row.Cells(0).Value.ToString
                        ...
                        'E:Invoice Date 
                        .Range("K" & rowcounter.ToString()).Value = row.Cells(4).Value.ToString
                        'F:PO No , 
                        ...
                        'I:Quantity
                        .Range("U" & rowcounter.ToString()).Value = row.Cells(8).Value.ToString
                        'J:FOC
                        .Range("V" & rowcounter.ToString()).Value = row.Cells(9).Value.ToString
                        'L:LotNo
                        .Range("S" & rowcounter.ToString()).Value = row.Cells(11).Value.ToString
                        'M:Exp Date
                        .Range("T" & rowcounter.ToString()).Value = row.Cells(12).Value.ToString
                        'N:Unit Price
                        .Range("W" & rowcounter.ToString()).Value = row.Cells(13).Value.ToString
                        'O:Discount
                        'P:Net Amount
                        .Range("X" & rowcounter.ToString()).Value = row.Cells(15).Value.ToString
                    End With
                End If
            End If
            rowcounter = rowcounter + 1
