...
Public Class frmGBSubDist
    ...
    Private Sub InitialPath()
        Dim salesinvupdpath As String = System.Configuration.ConfigurationSettings.AppSettings.Item("GBRAWPath")
        Dim currentdate As String
    ...
    Private Sub SetPath()
        ...
        txtOutput.Text = txtOutput.Text.ToString().Replace(".xls", "") + "GB RAW DATA " + currentdate + ".xls"
    End Sub
    ...
    Private Sub DisplayColorMapping()
        ...
        Dim gridrowcounter As Integer = 1
        ...
        For Each row As DataGridViewRow In dgvSales.Rows
            If Not row.IsNewRow And gridrowcounter >= 5 Then
                'Check Item Data Mapping
                itemcodestr = row.Cells(18).Value.ToString
            ...
            gridrowcounter = gridrowcounter + 1
    ...
    Private Function GetItemCode(ByVal itemcode As String) As String
        ...
        Dim SearchName As New SqlParameter("@SearchName", SqlDbType.VarChar, 50) : SearchName.Direction = ParameterDirection.Input : SearchName.Value = "GB ITEM" : Params(0) = SearchName
    ...
    Private Function GetCustomerCode(ByVal itemcode As String) As String
        Dim BusinessObject As New BusinessLayer.clsFileMaintenance
        Dim Params(1) As SqlParameter
        Dim SearchName As New SqlParameter("@SearchName", SqlDbType.VarChar, 50) : SearchName.Direction = ParameterDirection.Input : SearchName.Value = "GB CUSTOMER" : Params(0) = SearchName
        Dim SearchCode As New SqlParameter("@SearchCode", SqlDbType.VarChar, 50) : SearchCode.Direction = ParameterDirection.Input : SearchCode.Value = itemcode : Params(1) = SearchCode
        ResetTable()
        BusinessObject.Sub_Show(ServerPath2, "Util_DataMap_Select", CommandType.StoredProcedure, RemoteDataSet, "Table1", Params)
        If RemoteDataSet.Tables("Table1").Rows.Count > 0 Then
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        If row > -1 Then
            ...
            myLoadedForm.txtAddress1.Text = dgvSales.Rows(row).Cells(4).Value.ToString
            myLoadedForm.txtAddress2.Text = dgvSales.Rows(row).Cells(5).Value.ToString
            myLoadedForm.txtShipTo.Text = dgvSales.Rows(row).Cells(7).Value.ToString
            myLoadedForm.txtShipAdd1.Text = dgvSales.Rows(row).Cells(8).Value.ToString
            myLoadedForm.txtShipAdd2.Text = dgvSales.Rows(row).Cells(9).Value.ToString

            'Item Details
            myLoadedForm.ItemCode = dgvSales.Rows(row).Cells(18).Value.ToString
            myLoadedForm.ItemName = dgvSales.Rows(row).Cells(19).Value.ToString
            myLoadedForm.txtUnit.Text = dgvSales.Rows(row).Cells(20).Value.ToString
            myLoadedForm.txtUnitPrice.Text = dgvSales.Rows(row).Cells(25).Value.ToString
            'Set DataMap Search column
            myLoadedForm.gbCustomer.Text = "GB CUSTOMER"
            myLoadedForm.gbItem.Text = "GB ITEM"
    ...
    Private Sub GenerateRawFormatted()
            ...
            For Each row As DataGridViewRow In dgvSales.Rows
                If Not row.IsNewRow And gridrowcounter >= 5 Then
                    ...
                    If itemcodestr <> "" Then
                        ...
                        Dim nunitprice As Double
                        dsitem = ItemHelper.Getitem(realitemcode)

                        With ws
                            'Set Dist_Code, SubDist_Code,Prin_Code
                            .Range("A" & rowcounter.ToString()).Value = "05"
                            .Range("B" & rowcounter.ToString()).Value = "GB01"
                            ...
                            'A:Invoice No 
                            .Range("J" & rowcounter.ToString()).Value = row.Cells(11).Value.ToString

                            'E:Invoice Date 
                            .Range("K" & rowcounter.ToString()).Value = row.Cells(12).Value.ToString
                            'F:PO No , 
                            .Range("L" & rowcounter.ToString()).Value = row.Cells(13).Value.ToString
                            ...
                            'L:LotNo
                            .Range("S" & rowcounter.ToString()).Value = row.Cells(21).Value.ToString
                            'M:Exp Date
                            .Range("T" & rowcounter.ToString()).Value = row.Cells(22).Value.ToString

                            'I:Quantity
                            .Range("U" & rowcounter.ToString()).Value = row.Cells(23).Value.ToString
                            'J:FOC
                            .Range("V" & rowcounter.ToString()).Value = row.Cells(24).Value.ToString
                            'N:Unit Price = net amount / qty
                            nunitprice = Convert.ToDouble(row.Cells(31).Value.ToString) / Convert.ToDouble(row.Cells(23).Value.ToString)
                            '.Range("W" & rowcounter.ToString()).Value = row.Cells(25).Value.ToString
                            .Range("W" & rowcounter.ToString()).Value = nunitprice.ToString

                            'O:Discount
                            'P:Net Amount
                            .Range("X" & rowcounter.ToString()).Value = row.Cells(31).Value.ToString
                        End With
                    End If
                    rowcounter = rowcounter + 1
                End If
        ...
        Else
            MsgBox("Please correct all rows with red background!")
        End If
    End Sub
