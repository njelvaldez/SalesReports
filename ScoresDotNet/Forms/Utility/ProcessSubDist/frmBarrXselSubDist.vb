...
Public Class frmBarrXselSubDist
    ...
    Private Sub InitialPath()
        Dim salesinvupdpath As String = System.Configuration.ConfigurationSettings.AppSettings.Item("BarrXselRAWPath")
        Dim currentdate As String
    ...
    Private Sub SetPath()
        ...
        txtOutput.Text = txtOutput.Text.ToString().Replace(".xls", "") + "BARRXSEL RAW DATA " + currentdate + ".xls"
    End Sub
    ...
    Private Sub DisplayColorMapping()
        ...
        For Each row As DataGridViewRow In dgvSales.Rows
            If Not row.IsNewRow Then
                ...
                itemcodestr = row.Cells(13).Value.ToString
                ...
                customercodestr = row.Cells(2).Value.ToString.Trim()
                customernamestr = row.Cells(3).Value.ToString.Trim()
                If customercodestr = "" Then
                    If customernamestr.Length >= 50 Then
                        customercodestr = customernamestr.Substring(0, 49)
                    Else
                        customercodestr = customernamestr
                    End If
                End If
    ...
    Private Function GetItemCode(ByVal itemcode As String) As String
        ...
        Dim SearchName As New SqlParameter("@SearchName", SqlDbType.VarChar, 50) : SearchName.Direction = ParameterDirection.Input : SearchName.Value = "BARRXSEL ITEM" : Params(0) = SearchName
    ...
    Private Function GetCustomerCode(ByVal itemcode As String) As String
        Dim BusinessObject As New BusinessLayer.clsFileMaintenance
        Dim Params(1) As SqlParameter
        Dim SearchName As New SqlParameter("@SearchName", SqlDbType.VarChar, 50) : SearchName.Direction = ParameterDirection.Input : SearchName.Value = "BARRXSEL CUSTOMER" : Params(0) = SearchName
        Dim SearchCode As New SqlParameter("@SearchCode", SqlDbType.VarChar, 50) : SearchCode.Direction = ParameterDirection.Input : SearchCode.Value = itemcode : Params(1) = SearchCode
        ResetTable()
        BusinessObject.Sub_Show(ServerPath2, "Util_DataMap_Select", CommandType.StoredProcedure, RemoteDataSet, "Table1", Params)
        If RemoteDataSet.Tables("Table1").Rows.Count > 0 Then
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        If row > -1 Then
            ...
            myLoadedForm.ItemCode = dgvSales.Rows(row).Cells(12).Value.ToString
            myLoadedForm.ItemName = dgvSales.Rows(row).Cells(13).Value.ToString
            'Set DataMap Search column
            myLoadedForm.gbCustomer.Text = "BARRXSEL CUSTOMER"
            myLoadedForm.gbItem.Text = "BARRXSEL ITEM"
            myLoadedForm.txtUnit.Text = dgvSales.Rows(row).Cells(14).Value.ToString
            myLoadedForm.txtUnitPrice.Text = dgvSales.Rows(row).Cells(19).Value.ToString
    ...
    Private Sub btnGenerateRawData_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGenerateRawData.Click
        Try
            ...
            KillExcellApp()
        Catch ex As Exception
            MsgBox("Error in Generating RAW Data : " & ex.Message)
        End Try
    End Sub
            ...
            If Not row.IsNewRow Then
                ...
                If itemcodestr <> "" Then
                    ...
                    With ws
                        ...
                        .Range("B" & rowcounter.ToString()).Value = "LZN01"
                        ...
                        'A:Invoice No 
                        .Range("J" & rowcounter.ToString()).Value = row.Cells(9).Value.ToString

                        'E:Invoice Date 
                        .Range("K" & rowcounter.ToString()).Value = row.Cells(10).Value.ToString
                        'F:PO No , 
                        .Range("L" & rowcounter.ToString()).Value = row.Cells(11).Value.ToString
                        ...
                        'L:LotNo
                        .Range("S" & rowcounter.ToString()).Value = row.Cells(16).Value.ToString
                        'M:Exp Date
                        .Range("T" & rowcounter.ToString()).Value = row.Cells(17).Value.ToString

                        'I:Quantity
                        .Range("U" & rowcounter.ToString()).Value = row.Cells(18).Value.ToString
                        'J:FOC
                        .Range("V" & rowcounter.ToString()).Value = row.Cells(19).Value.ToString
                        'N:Unit Price
                        .Range("W" & rowcounter.ToString()).Value = row.Cells(20).Value.ToString
                        'O:Discount
                        'P:Net Amount
                        .Range("X" & rowcounter.ToString()).Value = row.Cells(21).Value.ToString
                    End With
                End If
            End If
            rowcounter = rowcounter + 1
