...
Public Class frm518SubDistNew
    ...
    Private Sub DisplayColorMapping()
        ...
        For Each row As DataGridViewRow In dgvSales.Rows
            If Not row.IsNewRow Then
                ...
                'item description is only available, Column L=11
                itemcodestr = StringHelper.Get50Chars(row.Cells(11).Value.ToString.Trim)
                ...
                'Check Customer Data Mapping , Col J = 9
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        If row > -1 Then
            ...
            'Item Description , Column L = 11
            myLoadedForm.ItemName = dgvSales.Rows(row).Cells(11).Value.ToString

            myLoadedForm.gbCustomer.Text = "518 CUSTOMER"
            myLoadedForm.gbItem.Text = "518 ITEM"
            myLoadedForm.txtUnit.Text = "" ' 518 no unit
            'Unit Price Column O = 13
            myLoadedForm.txtUnitPrice.Text = dgvSales.Rows(row).Cells(14).Value.ToString
    ...
    Private Sub GenerateRawFormatted()
        ...
        Try
            ...
            For Each row As DataGridViewRow In dgvSales.Rows
                If Not row.IsNewRow Then
                    'Get real Item Code , Item Desc Column L = 11
                    ...
                    If itemcodestr <> "" Then
                        ...
                        'only customer name is available , Customer Name , Column J = 9
                        customercodestr = StringHelper.Get50Chars(row.Cells(9).Value.ToString.Trim)
                        realcustomercode = GetCustomerCode(customercodestr)
                        ...
                        With ws
                            ...
                            .Range("K" & rowcounter.ToString()).Value = Convert.ToDateTime(invdate).ToShortDateString
                            'F:PO No , 
                            ...
                            'I:Quantity , Column M = 12
                            .Range("U" & rowcounter.ToString()).Value = row.Cells(12).Value.ToString
                            'J:FOC , Column N = 13
                            .Range("V" & rowcounter.ToString()).Value = row.Cells(13).Value.ToString
                            'L:LotNo
                            .Range("S" & rowcounter.ToString()).Value = ""
                            'M:Exp Date
                            .Range("T" & rowcounter.ToString()).Value = "12/31/1999"
                            'N:Unit Price , Column O = 14
                            .Range("W" & rowcounter.ToString()).Value = row.Cells(14).Value.ToString
                            'O:Discount
                            'P:Net Amount , Column P = 15
            ...
            xl = Nothing
        Catch ex As Exception
            MsgBox(ex.Message.ToString + " InnerException : " + ex.InnerException.ToString)
        End Try
