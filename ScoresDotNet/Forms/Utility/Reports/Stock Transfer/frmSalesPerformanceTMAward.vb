...
Public Class frmSalesPerformanceTMAward
    ...
    Private Sub SetPath()
        ...
        txtTemplate.Text = salesinvupdpath + "Sales Performance Team Manager.xls"
        txtOutput.Text = salesinvupdpath + "Sales Performance Team Manager " + currentdate + ".xls"
    End Sub
    ...
    Private Sub ShowInvoice(generateonly As Boolean)
        Try
            ...
            BusinessObject.Sub_Show(ServerPath2, "Util_SalesPerformanceTMAward", CommandType.StoredProcedure, RemoteDataSet, "IntegratedSales", Params)
            ...
            LogHelper.InsertLog("frmSalesPerformanceTMAward.ShowInvoice.Success")
        Catch ex As Exception
            MsgBox(ex.Message)
            LogHelper.InsertLog("frmSalesPerformanceTMAward.ShowInvoice.Error: " & ex.Message)
        End Try
    End Sub

    Private Sub ShowInvoiceCombi(generateonly As Boolean)
        Try
            If RemoteDataSet.Tables("IntegratedSales").Rows.Count > 0 Then RemoteDataSet.Tables("IntegratedSales").Clear()
            Dim BusinessObject As New BusinessLayer.clsFileMaintenance
            Dim commdate1 As DateTime
            ...
            Dim commissiondate1 As New SqlParameter("@commissiondate1", SqlDbType.DateTime, 10) : commissiondate1.Direction = ParameterDirection.Input : commissiondate1.Value = commdate1 : Params(0) = commissiondate1
            Dim commissiondate2 As New SqlParameter("@commissiondate2", SqlDbType.DateTime, 10) : commissiondate2.Direction = ParameterDirection.Input : commissiondate2.Value = commdate2 : Params(1) = commissiondate2
            BusinessObject.Sub_Show(ServerPath2, "Util_SalesPerformanceTMAwardCombi", CommandType.StoredProcedure, RemoteDataSet, "IntegratedSales", Params)
            dgvSales.DataSource = RemoteDataSet.Tables("IntegratedSales")
            gbSales.Text = "Total No Of Records : " + RemoteDataSet.Tables("IntegratedSales").Rows.Count.ToString
            LogHelper.InsertLog("frmSalesPerformanceTMAward.ShowInvoice.Success")
        Catch ex As Exception
    ...
    Private Sub GenerateSales(ByVal strFileName As String)
        ...
        Try
            ...
            'commissiondate, division, b.supname, sum(originalsales) [originalsales], sum(originaltarget) [originaltarget], 1 [NoOfMonths]
            For Each dr As DataRow In dt.Rows
                ...
                'supname, 
                strArr(rownumber, 2) = dr("supname").ToString
                'originalsales, 
            ...
            'ws.Range("A3").Value = "January - " + monthname + " " + dtpMonthTo.Value.Year.ToString()
            'Refresh PivotTable3
            ...
            xl.Quit()
            xl = Nothing
            MessageBox.Show("Sales Performance TM Award Report generation is complete!")
        Catch ex As Exception
            MessageBox.Show(ex.Message, "frmSalesPerformanceTMAward.GenerateSalesTrend module")
        Finally
        End Try
    End Sub

    Private Sub GenerateSalesCombi(ByVal strFileName As String)
        Dim xl As New Excel.Application
        Dim wb As Excel.Workbook
        Dim ws As Excel.Worksheet
        Dim wssalper As Excel.Worksheet
    ...
    Private Sub btnProcessCombi_Click(sender As Object, e As EventArgs) Handles btnProcessCombi.Click
        Dim salesinvupdfile As String = txtTemplate.Text.ToString
        'SetDate()
        Me.Cursor = Cursors.WaitCursor
        ShowInvoiceCombi(False)
        'this is now faster
        GenerateSalesCombi(salesinvupdfile)
        Me.Cursor = Cursors.Default
        KillExcellApp()
