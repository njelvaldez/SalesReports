...
Public Class frmBudgetForSpecialist
    ...
    End Sub
    Private Sub ShowPreviousBudget()
        Try
            Dim BusinessObject As New BusinessLayer.clsFileMaintenance
            Dim Params(0) As SqlParameter
            Dim RemoteDataset As New DataSet
            Dim salescycledate As New SqlParameter("@salescycledate", SqlDbType.DateTime, 10) : salescycledate.Direction = ParameterDirection.Input : salescycledate.Value = GetPreviousBudgetMonth() : Params(0) = salescycledate
            RemoteDataset.Tables.Add("TempTable")
            BusinessObject.Sub_Show(ServerPath2, "Util_BudgetFile_Select", CommandType.StoredProcedure, RemoteDataset, "TempTable", Params)
            dgvPreviousBudget.DataSource = RemoteDataset.Tables("TempTable")
            dgvPreviousBudget.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.AllCells
            LogHelper.InsertLog("frmBudgetForSpecialist.ShowPreviousBudget.Success")
        Catch ex As Exception
            MsgBox(ex.Message)
            LogHelper.InsertLog("frmBudgetForSpecialist.ShowPreviousBudget.Error: " & ex.Message)
        End Try
    End Sub

    Private Function GetPreviousBudgetMonth() As String
        Dim retval As String = ""
        Try
            retval = Convert.ToString(dtpMonth.Value.Month - 1) + "/1/" + Convert.ToString(dtpMonth.Value.Year)
        Catch ex As Exception
    ...
    Private Sub btnOpenBudgetFromList_Click(sender As Object, e As EventArgs) Handles btnOpenBudgetFromList.Click
        ...
        EnableSaveBudget(True)
    End Sub
    Private Function GetMRCodePromPreviousBudget()
        Dim BusinessObject As New BusinessLayer.clsFileMaintenance
        Dim Params(2) As SqlParameter
        Dim retval As String = ""
        'txtBudgetPath.Text, TabControl1.SelectedTab.Name, txtMrCode.Text, dtpMonth.Value
        Dim filename As New SqlParameter("@filename", SqlDbType.VarChar, 250) : filename.Direction = ParameterDirection.Input : filename.Value = txtBudgetPath.Text : Params(0) = filename
        Dim sheetname As New SqlParameter("@sheetname", SqlDbType.VarChar, 150) : sheetname.Direction = ParameterDirection.Input : sheetname.Value = TabControl1.SelectedTab.Name : Params(1) = sheetname
        Dim salescycledate As New SqlParameter("@salescycledate", SqlDbType.DateTime, 10) : salescycledate.Direction = ParameterDirection.Input : salescycledate.Value = GetPreviousBudgetMonth() : Params(2) = salescycledate
        Dim RemoteDataset As New DataSet
        Try
            RemoteDataset.Tables.Add("TempTable")
            BusinessObject.Sub_Show(ServerPath2, "Util_BudgetFile_SelectMRCode", CommandType.StoredProcedure, RemoteDataset, "TempTable", Params)
            If RemoteDataset.Tables("Temptable").Rows.Count > 0 Then
                retval = RemoteDataset.Tables("Temptable").Rows(0)("MRCODE").ToString()
            Else
                retval = ""
            End If
        Catch ex As Exception
    ...
    Private Sub TabControl1_Click(sender As Object, e As EventArgs) Handles TabControl1.Click
        txtMrCode.Text = GetMRCodePromPreviousBudget()
    End Sub

    Private Sub txtMrCode_TextChanged(sender As Object, e As EventArgs) Handles txtMrCode.TextChanged
        txtSpecialist.Text = GetSpecialistName(txtMrCode.Text)
    End Sub

    Private Function GetSpecialistName(mrcodepar As String)
        Dim retval As String
        Dim BusinessObject As New BusinessLayer.clsFileMaintenance
        Dim Params(0) As SqlParameter
        Dim mrcode As New SqlParameter("@mrcode", SqlDbType.VarChar, 10) : mrcode.Direction = ParameterDirection.Input : mrcode.Value = mrcodepar : Params(0) = mrcode
        Dim RemoteDataset As New DataSet
        RemoteDataset.Tables.Add("TempTable")
        BusinessObject.Sub_Show(ServerPath2, "Util_Medrep_Select", CommandType.StoredProcedure, RemoteDataset, "TempTable", Params)
        If RemoteDataset.Tables("Temptable").Rows.Count > 0 Then
            retval = RemoteDataset.Tables("Temptable").Rows(0)("MRNAME").ToString()
        Else
            retval = ""
        End If
        Return retval
